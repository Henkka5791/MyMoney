...
def monthly(day_from,day_to):
    ...
    sql = '''SELECT 
                ...
                COALESCE(b.budget,0),
                COALESCE(t.income+t.outcome,0) AS balance,
                COALESCE(b.budget+t.outcome,0) AS budget_balance
            FROM 
                (SELECT 
            ...
            FULL OUTER JOIN (
                SELECT 
                    ...
                    b.period AS period,
                    sum(b.amount) AS budget from budgets b, 
                    categories c 
                WHERE 
                    b.category_id=c.id 
                    AND c.account_id=:id 
...
def total_sum(day_from,day_to):
    ...
    sql = '''SELECT 
                ...
                sum(COALESCE(b.budget,0)),
                sum(COALESCE(t.income+t.outcome,0)) AS balance,
                sum(COALESCE(b.budget+t.outcome,0)) AS budget_balance
            FROM 
                (SELECT 
                    EXTRACT(year FROM t.created_at) AS year,
                    EXTRACT(month FROM t.created_at) AS month,
